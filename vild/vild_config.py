...
class AudioViLDConfig:
    def __init__(self, mark_version="mark4.1"):  # change needed
        ...
        # [mark4.4 선정 2026-08-02] 0.45 -> 0.44. machine_noise 프롬프트 수정 후 재학습한
        # 모델의 val(430) 스윕(0.05~0.95, 0.01 간격) 결과. 재현 검증 thr=0.45로 430/430 일치.
        # accuracy 최고 0.8814 구간 3곳 중 폭이 가장 넓은 (0.4391, 0.4487](폭 0.0096, 중앙 0.4439)
        # 채택. 나머지 두 곳은 폭 0.0051 / 0.0007로 훨씬 좁음.
        # val 성능: accuracy 0.8814 / macro F1 0.8814 / machine_noise P 0.869 R 0.898 / others FPR 0.135
        #
        # ⚠️ 이 버전은 8개 중 분리력이 가장 낮음(ROC AUC 0.9487, 나머지는 0.973~0.9997).
        #    threshold 조정으로 얻는 이득이 0.2%p뿐이며(0.8791 -> 0.8814) 어느 지점을 잡아도
        #    precision과 recall이 정면으로 맞바뀜. 원인은 운영점이 아니라 데이터 구성임:
        #    타겟은 AI Hub A_3_c/d/e/f 4종인데 others 풀에 같은 A_3 대분류의 a·b가 들어가 있어
        #    (오탐률 A_2_a 21.4% / A_3_a 17.2% / A_3_b 14.3% vs 나머지 1.0%) 라벨만 반대인
        #    유사 소음이 섞여 있음. 프롬프트 충돌(appliance 어휘) 수정으로 FPR은 0.191->0.135로
        #    개선됐으나 AUC는 0.9415->0.9487로 0.007만 올라, 프롬프트는 원인의 일부였을 뿐임.
        #    others 재구성은 4.4만 조건이 달라져 버전 간 비교가 깨지므로 보류함.
        #   - 대안 0.53: acc 0.8791, FPR 0.102 (오탐 최소화 우선 시)
        #
        # [test 결과 2026-08-02] val로 고른 0.44의 test 성능: accuracy 0.9093 / ROC AUC 0.9682 /
        # P 0.9112 / R 0.9070 / others FPR 0.0884. val(0.8814) 대비 +2.8%p로 test가 더 높았고,
        # test 재선정 시 최고가 0.9163이라 threshold 손실은 0.7%p뿐. 재현 검증 430/430 일치.
        # ⚠️ 속도 측정치는 다른 버전과 직접 비교 불가 — 이 버전만 코랩 T4 런타임에서 측정됐고
        #    (전체 중앙 40.3ms: 이미지화 12.4 + 추론 28.0) 나머지 6개는 A100 런타임 기준
        #    20.2~26.9ms임. 코랩은 GPU 타입에 따라 할당 CPU 사양도 달라져 CPU 추론 속도가 변함.
        #    모델 성능 지표에는 영향 없음. 논문용 수치는 8개를 같은 런타임에서 재측정할 예정.
        self.target_decision_threshold = 0.44
